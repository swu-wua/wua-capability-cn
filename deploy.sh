#!/usr/bin/env bash
# One-shot deploy for the CN variant.
# Same flow as ksa/uk: copy → git init → GitHub repo → Vercel deploy.

set -euo pipefail

REPO_NAME="wua-capability-cn"
REPO_VISIBILITY="public"
DEST_DIR="$HOME/Documents/$REPO_NAME"

SRC_DIR="$HOME/Library/Application Support/Claude/local-agent-mode-sessions/879fe105-5eae-41f9-aa48-70571c0bee56/b8833c33-87cd-4a94-8044-20a313b79f26/local_af6330e8-f853-4707-99cb-8a2ce0930ab3/outputs/wua-site-cn"
...
if git diff --cached --quiet; then
  ...
  git -c user.email="${USER}@local" -c user.name="$USER" commit -m "Initial commit — CN bilingual capability statement" >/dev/null
...
DEPLOY_URL=$(vercel --prod --yes 2>&1 | tee /tmp/vercel-cn.log | grep -Eo "https://[a-zA-Z0-9.-]+\.vercel\.app" | tail -1)
...
echo
echo "Next: in Vercel dashboard → wua-capability-cn → Settings → Domains, add"
echo "      bd-cn.wuastudio.com  (then add the CNAME at your DNS registrar)"
